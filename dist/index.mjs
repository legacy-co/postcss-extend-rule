...
const nesting = postcssNesting(); // functional selector match

const functionalSelectorMatch = /(^|[^\w-])(%[_a-zA-Z]+[_a-zA-Z0-9-]*)([^\w-]|$)/i; // plugin

var index = postcss.plugin('postcss-extend-rule', rawopts => {
  ...
  return (root, result) => {
    ...
    const nestings = []; // for each extend at-rule
    ...
    root.walkAtRules(extendMatch, extendAtRule => {
      ...
      if (!extendedAtRules.has(extendAtRule)) {
        ...
        const selectorIdMatch = getSelectorIdMatch(extendAtRule.params); // extending rules
